...
static int create_qi_desc(struct pkvm_iommu *iommu)
{
	...
	qi->desc = pkvm_phys_to_virt(IQ_DESC_BASE_PHYS(viommu->iqa));
	order = ecap_smts(iommu->iommu.ecap) ? 1 : 0;
	__pkvm_host_donate_hyp(IQ_DESC_BASE_PHYS(viommu->iqa), (1 << order) * PAGE_SIZE);
...
#else
	iommu->viommu.pgt.root_pa = vreg->rta & VTD_PAGE_MASK;
#endif
