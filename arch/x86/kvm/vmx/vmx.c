				    ...
				    struct vmx_capability *vmx_cap)
{
	struct vmcs_config_setting setting = {
		...
		.cpu_based_exec_ctrl_opt =
			CPU_BASED_TPR_SHADOW |
			CPU_BASED_USE_MSR_BITMAPS |
			CPU_BASED_ACTIVATE_SECONDARY_CONTROLS,
		.cpu_based_2nd_exec_ctrl_min = 0,
		...
		.pin_based_exec_ctrl_opt =
			PIN_BASED_VIRTUAL_NMIS |
			PIN_BASED_POSTED_INTR |
			PIN_BASED_VMX_PREEMPTION_TIMER,
		.has_broken_vmx_preemption_timer =
			cpu_has_broken_vmx_preemption_timer(),
		.vmentry_ctrl_min = VM_ENTRY_LOAD_DEBUG_CONTROLS,
		.vmentry_ctrl_opt =
			VM_ENTRY_LOAD_IA32_PERF_GLOBAL_CTRL |
	...
	if (cpu_has_sgx())
		setting.cpu_based_2nd_exec_ctrl_opt |=
	...
	if (boot_cpu_data.x86 == 0x6) {
		...
		case 46: /* BA97 */
			setting.perf_global_ctrl_workaround = true;
			break;
		default:
			break;
		}
