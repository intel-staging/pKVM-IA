...
static void *its_allocate_thunk(void *addr, int reg)
{
	int size = 3 + (reg / 8);
	void *thunk;

	/*
	 * For the pkvm hypervisor code, fallback to the static thunks rather
...
{
	u8 *thunk = __x86_indirect_its_thunk_array[reg];
	u8 *tmp = its_allocate_thunk(addr, reg);
...
static inline uintptr_t get_pkvm_offset(void *dest)
{
#define GEN(reg)								\
	if (dest == __x86_indirect_thunk_##reg)					\
		return (void *)&__x86_indirect_thunk_##reg##__pkvm -		\
		       (void *)&__x86_indirect_thunk_##reg;
#include <asm/GEN-for-each-reg.h>
#undef GEN

#ifdef CONFIG_MITIGATION_CALL_DEPTH_TRACKING
#define GEN(reg)								\
	if (dest == __x86_indirect_call_thunk_##reg)				\
		return (void *)&__x86_indirect_call_thunk_##reg##__pkvm -	\
		       (void *)&__x86_indirect_call_thunk_##reg;
#include <asm/GEN-for-each-reg.h>
#undef GEN

#define GEN(reg)								\
	if (dest == __x86_indirect_jump_thunk_##reg)				\
		return (void *)&__x86_indirect_jump_thunk_##reg##__pkvm -	\
		       (void *)&__x86_indirect_jump_thunk_##reg;
#include <asm/GEN-for-each-reg.h>
#undef GEN
#endif
	...
	if (dest == srso_alias_return_thunk)
		return srso_alias_return_thunk__pkvm - srso_alias_return_thunk;
	if (dest == srso_return_thunk)
		return srso_return_thunk__pkvm - srso_return_thunk;
#endif
#ifdef CONFIG_MITIGATION_UNRET_ENTRY
	if (dest == retbleed_return_thunk)
		return retbleed_return_thunk__pkvm - retbleed_return_thunk;
#endif
#ifdef CONFIG_MITIGATION_CALL_DEPTH_TRACKING
	if (dest == call_depth_return_thunk)
		return call_depth_return_thunk__pkvm - call_depth_return_thunk;
#endif
	if (dest == __x86_return_thunk)
		return __x86_return_thunk__pkvm - __x86_return_thunk;
#ifdef CONFIG_MITIGATION_ITS
#define GEN(reg)								\
	if (dest == __x86_indirect_its_thunk_##reg)				\
		return (void *)&__x86_indirect_its_thunk_##reg##__pkvm -	\
		       (void *)&__x86_indirect_its_thunk_##reg;
#include <asm/GEN-for-each-reg.h>
#undef GEN
	if (dest == its_return_thunk)
		return its_return_thunk__pkvm - its_return_thunk;
#endif
#endif
	return 0;
}

static void patch_pkvm(void *addr)
{
	struct insn insn;
	uintptr_t offset;
	void *dest;
	...
	if (WARN_ON_ONCE(insn_decode_kernel(&insn, addr) < 0))
		return;

	imm = insn.immediate.value;
	dest = addr + insn.length + imm;
	if (is_pkvm_text(dest))
		return;

	offset = get_pkvm_offset(dest);

	if (WARN_ON_ONCE(offset == 0)) {
		pr_err_ratelimited("%pS not found in pkvm's symbols\n", dest);
		return;
	}
	apply_reloc(4, &imm, offset);
	text_poke_early(addr + insn_offset_immediate(&insn), &imm, 4);
}
#else
static inline void patch_pkvm(void *addr) {}
...
void __init_or_module noinline apply_retpolines(s32 *start, s32 *end)
{
	...
	for (s = start; s < end; s++) {
		...
		/*
		 * Patch the pkvm hypervisor text following the same check condition
		 * as done by the patch_retpoline.
		 */
		if ((cpu_feature_enabled(X86_FEATURE_RETPOLINE) &&
		     !cpu_feature_enabled(X86_FEATURE_RETPOLINE_LFENCE)) ||
