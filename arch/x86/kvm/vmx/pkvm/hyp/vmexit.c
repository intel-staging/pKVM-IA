...
static unsigned long handle_vmcall(struct kvm_vcpu *vcpu)
{
	...
	case __pkvm__iommu_set_sm_ce:
		ret = pkvm_iommu_set_sm_ce(a0);
		break;
	case __pkvm__iommu_clear_pasid_entry:
		ret = pkvm_iommu_clear_pasid_entry(a0);
		break;
	case __pkvm__iommu_set_pasid_fl:
		ret = pkvm_iommu_set_pasid_fl(a0);
		break;
	case __pkvm__iommu_set_pasid_sl:
		ret = pkvm_iommu_set_pasid_sl(a0);
		break;
	case __pkvm__iommu_domain_alloc:
		ret = pkvm_iommu_domain_alloc(a0);
		break;
	case __pkvm__iommu_domain_free:
		ret = pkvm_iommu_domain_free(a0);
		break;
	case __pkvm__iommu_map_pages:
		ret = pkvm_iommu_domain_map(a0);
		break;
